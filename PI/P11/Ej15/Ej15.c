...
typedef struct moveToFrontCDT{
    ...
    int (*cmp) (elemType e1, elemType e2);  // Guardamos la función de comparación
}moveToFrontCDT;
...
*/
static TList addRec(TList list, elemType elem, int * flag, int (*cmp) (elemType e1, elemType e2));
...
** nodo en foundco
*/
static TList searchAndDisconnect(TList list ,elemType elem, TList * found, int (*cmp) (elemType e1, elemType e2));

/* Utilizamos calloc para cear un nuevo conjunto de elementos genérico ya que
** al inicio está vacío
*/
moveToFrontADT newMoveToFront(int (*cmp) (elemType e1, elemType e2)){
...
static TList searchAndDisconnectIter(TList list ,elemType elem, int (*cmp) (elemType e1, elemType e2)){
